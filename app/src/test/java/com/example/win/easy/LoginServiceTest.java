...
public class LoginServiceTest {
     ...
     * <p>前两个的验证比较直观，关于第三个回调函数有没有执行，这里设置的是给主线程发送一个信号，不然主线程就会一直等待</p>
    ...
    public void testLoginByPhone() throws InterruptedException {
        ...
        loginService.loginByPhone(loginPhone, truePassword,success-> {
            if(success)
                notifyMainThreadLoginReady();
        });
    ...
    public void testFailLogin() throws InterruptedException {
        ...
        loginService.loginByPhone(loginPhone,wrongPassword,success-> {
            if(!success)
                notifyMainThreadLoginReady();
        });
