...
public class DownloadServiceAdapterTest {
    ...
    public void setUp(){
        ...
        //初始化invoker
        invoker=new MockDownloadServiceInvoker(lifecycleOwner,downloadServiceAdapter);
        ...
        doAnswer(invocation -> {
            ...
            // 并新建一个线程模拟下载的子线程
            SongDO songDO=invocation.getArgument(0,SongDO.class);
            OnReadyFunc<SongDO> onReadyFunc=invocation.getArgument(1,OnReadyFunc.class);
        ...
        when(voUtil.toDO(any(SongVO.class))).thenAnswer(invocation -> {
            SongVO songVO=invocation.getArgument(0,SongVO.class);
    ...
    private void verifyLiveDataUpdateSuccessfullyAndCallbackIsInvokedRespectively() {
        ...
        System.out.println(invoker.getMySong().getSongFile().getAbsolutePath());
    ...
    private void notifyDatabaseUpdateToFinish(){
        ...
        synchronized (mainThreadSignalToIndicateDatabaseUpdateShouldBeFinished){
            ...
            mainThreadSignalToIndicateDownloadShouldBeFinished.notify();
        }
    }
    ...
    @Rule MockAndroidEnvironmentMainLooperRule rule=new MockAndroidEnvironmentMainLooperRule();
 ...
 * <p>主要的方法就是那个{@link MockDownloadServiceInvoker#simulateInvokingDownloadService()}模拟一次调用</p>
...
//    private final BooleanSemaphore mainThreadSignalToIndicateDatabaseUpdateShouldBeFinished;
    ...
    public void update(SongDO songDO) {
        ...
        //模拟更新物理数据
        dbAccess.execute(this::notifyMainThreadThatSubThreadBeginsToUpdateDatabaseAndThenPretendToBeUpdatingDatabaseUntilSignalFromMainThread);
